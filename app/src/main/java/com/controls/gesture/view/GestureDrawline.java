package com.controls.gesture.view;
...
import com.controls.gesture.R;
import com.controls.gesture.Util.GestureUtil;
import com.controls.gesture.Util.MD5Util;
import com.controls.gesture.entity.GesturePoint;
...
public class GestureDrawline extends View {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (currentPoint != null) {
                    currentPoint.setPointState(GestureUtil.POINT_STATE_SELECTED);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (pointAt == null || currentPoint.equals(pointAt) || GestureUtil.POINT_STATE_SELECTED == pointAt.getPointState()) {
                    ...
                    pointAt.setPointState(GestureUtil.POINT_STATE_SELECTED);

                    // 判断是否中间点需要选中
                    GesturePoint betweenPoint = getBetweenCheckPoint(currentPoint, pointAt);
                    if (betweenPoint != null && GestureUtil.POINT_STATE_SELECTED != betweenPoint.getPointState()) {
                        // 存在中间点并且没有被选中
                        ...
                        // 设置中间点选中
                        betweenPoint.setPointState(GestureUtil.POINT_STATE_SELECTED);
                        // 赋值当前的point;
                        currentPoint = pointAt;
                    } else {
    ...
    final class clearStateRunnable implements Runnable {
        public void run() {
            ...
            for (GesturePoint p : list) {
                p.setPointState(GestureUtil.POINT_STATE_NORMAL);
            }
            invalidate();
            isDrawEnable = true;
        }
    ...
    private void drawErrorPathTip() {
        ...
        if(first !=null){
            first.setPointState(GestureUtil.POINT_STATE_WRONG);
        }
        for (Pair<GesturePoint, GesturePoint> pair : lineList) {
            pair.first.setPointState(GestureUtil.POINT_STATE_WRONG);
            pair.second.setPointState(GestureUtil.POINT_STATE_WRONG);
            canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(),
                    pair.second.getCenterX(), pair.second.getCenterY(), paint);// 画线
        }
        invalidate();
